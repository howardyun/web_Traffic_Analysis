...
int main(int argc, char** argv) {
	...
	char* com_data_name = argv[3];

	read_options(string(optionname));

	string data_path = string(d["OUTPUT_LOC"]) + string(data_name)+'/';
	string trainlist  = data_path + "trainlist";
	string testlist = string(com_data_name) + "testlist";
	...
	FEAT_NUM = get_featnum(data_path);
	cout<<FEAT_NUM<<endl;
	//learn weights
