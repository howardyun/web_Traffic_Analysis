...
if __name__ == '__main__':
    ...
    if os.path.exists(trainfeaturepath):
        ...
        logger.debug(call("python extract.py "+args.train, shell = True))
    if os.path.exists(testfeaturepath):
        logger.debug("skip test set extraction")
    else:
        logger.debug(call("python extract.py "+args.test +" -num "+ args.num, shell = True))


    if os.path.exists(modelpath):
        logger.debug("skip train model")
    else:
        logger.debug(call("python train.py " +trainfeaturepath, shell = True))

    logger.info(call("python evaluate.py "+testfeaturepath + " -m "+modelpath, shell = True))
