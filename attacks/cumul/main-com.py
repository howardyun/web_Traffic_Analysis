...
def parse_arguments():
    ...
    parser.add_argument('compare_path',
                        metavar='<compare path>',
                        help='Path to the directory of the compare features')
...
def GridSearch(train_X,train_Y):
    ...
    clf = GridSearchCV(estimator = SVC(kernel = 'rbf'), param_grid = param_grid, \
        scoring = my_scorer, cv = 5, verbose = 2, n_jobs = 8)
...
if __name__ == '__main__':
    ...
    com = np.load(args.compare_path,allow_pickle=True).item()
    X = np.array(dic['feature'])
    y = np.array(dic['label'])
    X_com = np.array(com['feature'])
    y_com = np.array(com['label'])
    if not OPEN_WORLD:
        X = X[y<MON_SITE_NUM]
        y = y[y<MON_SITE_NUM]
    print(X.shape, y.shape)   
    print(X_com.shape, y_com.shape)

    #normalize the data
    scaler = preprocessing.MinMaxScaler((-1,1))
    X = scaler.fit_transform(X)  
    X_com = scaler.fit_transform(X_com)
    ...
    folder_num = 0
    flag = 1
    for train_index, test_index in sss.split(X_com,y_com):
        logger.info('Testing fold %d'%folder_num)
        folder_num += 1
        # print("TRAIN:", train_index, "TEST:", test_index)
        X_train, X_test = X[train_index], X_com[test_index]
        y_train, y_test = y[train_index], y_com[test_index]
